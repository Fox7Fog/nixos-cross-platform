...

{
  ...
  system-updater = pkgs.buildGoModule {
    ...
    vendorHash = pkgs.lib.fakeSha256; # Nix will prompt for the correct hash on first build
  ...
  setup-dev-shells = pkgs.buildGoModule {
    ...
    vendorHash = pkgs.lib.fakeSha256; # Nix will prompt for the correct hash on first build if needed
